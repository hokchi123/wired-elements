...
(async () => {
  ...
  // Check hand-drawn fonts loaded
  const fontCheck = consoleMsgs.some(m => m.includes('手绘字体已就绪'));

  // Background not pure green
  ...
  // Take 2 screenshots to compare jitter differences
  await page.screenshot({ path: 'sketch-1.png' });
  await page.waitForTimeout(500);
  // Reload to get new jitter seed
  await page.reload({ waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(10000);
  await page.screenshot({ path: 'sketch-2.png' });

  // Compare screenshots (the map/NPCs are same, but jitter on UI boxes should differ)
  const compareResult = await page.evaluate(() => {
    // Just verify the page loaded OK after reload
    const c = document.getElementById('aiTownCanvas');
    return c ? true : false;
  });

  await page.screenshot({ path: 'ai-town-v4.png' });

  console.log('\n========== 验收结果 ==========');
  console.log(`✅ Canvas 动画: ${animating ? '通过' : '失败'}`);
  console.log(`✅ 手绘字体加载: ${fontCheck ? '通过' : '失败'}`);
  console.log(`✅ 背景非纯绿: rgba(${bg.r},${bg.g},${bg.b}) ${!(bg.g > 100 && bg.r < 50 && bg.b < 50) ? '通过' : '失败'}`);
  console.log(`✅ UI可见: nav=${nav}, drop=${dropText}, info=${info} ${nav && dropText && info ? '通过' : '失败'}`);
  console.log(`✅ 刷新后页面正常: ${compareResult ? '通过' : '失败'}`);
